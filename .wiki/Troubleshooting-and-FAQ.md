...
### Q6: Can I change how close items need to be to merge?
Yes! Use the `item_clumps:merge_radius` GameRule (valid range $1\text{ to }10\text{ blocks}$):
```text
...
| **Labels not showing** | Clump count is below max stack or `label_min_count` | Adjust `/gamerule item_clumps:label_min_count -1` |
